    ...
    <div class="row de-cont">
        <div class="row">
            ...
            <div class="col de-info">
                ...
                <p><i>{{ $user->role }}</i></p>
                <p><i class="fa-solid fa-envelope-open-text" style="margin-right: 5px"></i>{{ $user->email }}</p>
                <p><i class="fa-solid fa-square-phone" style="margin-right: 5px"></i>{{ $user->phone }}</p>
                <div class="send-mess" >
                    <div class="header">
                        Send message
                    </div>
                    <div class="card">
                        <div class="content table-responsive table-full-width">
                            <form action="{{route('message.store')}}" method="POST" style="margin: 0px 20px">
                                @csrf
                                @if ($message = Session::get('id')) 
                                <input type="hidden" name="id" value="{{ $message }}">
                                @endif
                                <input type="hidden" name="receive_id" value="{{ $user->id }}">
                                <input type="hidden" name="receive_name" value="{{ $user->hoten }}">
                    ...
                    <div class="content table-responsive table-full-width">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                                ...
                                @else
                                    @foreach ($mess_sends as $mess_send)
                                    <tr>
                                        <td>{{$mess_send->updated_at}}</td>
                                        <td style="text-align: justify;">{{$mess_send->content}}</td>
                                        <td>
                                            <a href="{{route('message.edit', $mess_send)}}"><div class="edit" style="background-color: #03A9F4;">edit</div></a>
                                            <form action="{{route('message.destroy', $mess_send)}}" method="post" style="display:inline-block">
                                                @method("DELETE")
                                                @csrf
